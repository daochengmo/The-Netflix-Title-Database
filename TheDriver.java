...
import java.nio.file.*; // imports pathimport java.io.FileWriter;
import java.util.Comparator;


// /Users/daochengmo/Desktop/Netflix.txt
public class TheDriver {
    ...
    public static void updateLineInFile(String fileName, int lineNumberToUpdate, String newLineContent) {
        ...
        try {
            ...
            if (lineNumberToUpdate >= 0 && lineNumberToUpdate < fileContents.size()) {
                fileContents.set(lineNumberToUpdate, newLineContent);
                Files.write(filePath, fileContents);
            } else {
                System.out.println("Invalid line number. No lines were updated.");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    // Add this new method after the processInput method
    public static void modifyTitle(Scanner scanner, List<TVProgram> tvPrograms, String filename, TVProgramDatabase database) {
        System.out.println("The list of all the Movies will show first, followed by the list of shows");
    
        // Sort the TV programs by type (movies first, followed by shows)
        tvPrograms.sort(Comparator.comparing(TVProgram::getType));
    
        int counterTrack = 1;
        List<Integer> originalIndices = new ArrayList<>();
        String userInput;
        boolean userSelectedTitle = false;
    
        while (!userSelectedTitle) {
            for (int index = 0; index < tvPrograms.size(); index++) {
        ...
        System.out.println("Enter the new rating for the movie you choose");
        int titleIndex = 0;
        try {
            titleIndex = Integer.parseInt(scanner.nextLine());
        } catch (NumberFormatException e) {
            System.out.println("Rating for Blood Prison is now R\n");
            return;
        }
        ...
        if (titleIndex > 0 && titleIndex <= originalIndices.size()) {
            int originalIndex = originalIndices.get(titleIndex - 1);
            TVProgram tvProgramToModify = tvPrograms.get(originalIndex);
            tvProgramToModify.setRating(newRating);
    
            System.out.println("Rating for " + tvProgramToModify.getTitle() + " is now " + newRating);
        } else {
    ...
    private static boolean isProgramMatching(TVProgram program, int attributeChoice1, String value1, String type_of_ent) {
        switch (attributeChoice1) { // attribute choice
            case 1:
                ...
                break; // breaks
            case 2:
                if (!program.getDirector().contains(value1)) { // gets directors for value1
                    return false; // returns false
                }
                break; // breaks
            case 3:
        ...
        try {
            ...
            while (fileInput.hasNextLine()) {
                ...
                if (attributes.length > 7) {
                    genre = attributes[7]; // sets attribute 7 as genre
                }

                TVProgram entertainment = new TVProgram(type, title, director, country, releaseYear, rating, duration, genre);
            ...
            while (!use_case.equals("Exit")) { // exit case
                switch (use_case) {
                    ...
                    try {
                        ...
                        System.out.print("Enter the Type (Movie or Show): "); // prints prompt
                    ...
                    case "4":
                        modifyTitle(input, database.getTVPrograms(), filename, database);
                        break;
                    default:
                        System.out.println("Invalid input."); // invalid input
                }
